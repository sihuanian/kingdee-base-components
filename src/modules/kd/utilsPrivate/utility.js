 ...
 * @param {boolean} copyFunctions - wether to copy functions, if false returns noop fn
 * @returns {object|array} copy of the item
 */
export function deepCopy(obj, copyFunctions = true) {
    if (Object(obj) !== obj) {
        // primitives
        return obj;
    }
    ...
    if (Array.isArray(obj)) {
        ...
        for (let i = 0; i < len; i++) {
            obj2.push(deepCopy(obj[i], copyFunctions));
        }
        return obj2;
    }
    ...
    for (let i = 0; i < len; i++) {
        const key = keys[i];
        result[key] = deepCopy(obj[key], copyFunctions);
    }
    return result;
}
